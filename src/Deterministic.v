...
Theorem det_complete `{Deterministic A B T r t} : forall s s',
    s' = t s ->
    r s s'.
Proof.
  intros.
...
Ltac det_exact_any :=
  match goal with
  | [ Hdet: Deterministic ?r ?t,
            Hr: ?r _ _ |- _ ] => apply (@det_exact _ _ _ r t Hdet) in Hr
...
Instance zoom_Deterministic
         A C (lens: Lens A C) (lens_wf: LensWf lens) T (r: relation C C T) f
         {Hr: Deterministic r f}
  : Deterministic (zoom proj inj r)
                  (fun (x:A) => match f (proj x) with
