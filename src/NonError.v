...
Instance nonError_bind
  `{NonError A B T1 r1} `{forall x, @NonError B C T2 (r2 x)}:
  NonError (and_then r1 r2).
Proof.
  unfold NonError, not, and_then in *.
  (intuition idtac); propositional; eauto.
Qed.

Instance nonError_or
         `{NonError A B T r1} `{!NonError r2} :
  NonError (r1 + r2)%rel.
Proof.
...
Instance nonError_star
         `{NonError A A T r1} :
  NonError (seq_star r1).
Proof.
...
Instance nonError_bind_star
         `{forall x, @NonError A A T (r1 x)} :
